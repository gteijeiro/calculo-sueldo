...
const MODULOS = [
  { id: 'lct',    label: '💼 Relación de dependencia', sublabel: 'LCT / Ley 27.743', color: 'var(--blue)' },
  { id: 'dom',    label: '🏠 Personal doméstico',       sublabel: 'Ley 26.844',       color: 'var(--orange-dark)' },
];

export default function App() {
  const [modulo, setModulo] = useState(() => localStorage.getItem('modulo') || 'lct');
  ...
  const mod = MODULOS.find(m => m.id === modulo) ?? MODULOS[0];
  ...
  return (
    <>
      <div className="header">
        ...
        <p>Período Fiscal 2026 · Valores oficiales ARCA</p>
      ...
      {modulo === 'lct' && (
        <AppLCT showJsonMap={showJsonMap} setTheme={setTheme} theme={theme} />
      )}
      {modulo === 'dom' && (
        <AppLey26844 showJsonMap={showJsonMap} />
      )}
    </>
  );
}
